...
function gameObject:createFromXML(xml_data, sprite_data, map, texture, debug)
...
function gameObject:createFromFile(filename, x, y)
  ...
  if obj.texture ~= nil then
    print("GameObject> Texture " .. filename .. " loaded.", obj.texture.w, obj.texture.h)
    obj.textureName = filename
    obj.size:set(obj.texture.w, obj.texture.h)
    obj.hitSize = obj.size:length() * 0.5
  end
...
function getTextureData(sprite_data, filename, trim)
  ...
  local f, us, vs, u, v
  for i, _ in ipairs(sprite_data["frames"]) do
    f   = sprite_data["frames"][i]
    if f.filename == filename then 
      us  = f.frame.w / img_w
      vs  = f.frame.h / img_h
      u   = f.frame.x / img_w
      v   = f.frame.y / img_h
      return u, v, us, vs, img_w, img_h, f.sourceSize.w, f.sourceSize.h
    end
  end
  return nil
end
