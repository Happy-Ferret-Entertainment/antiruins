...
--[[
if platform == "DC" then
  vec3.distance = function(v, u)
      return vec3.sub(v, u, vtmp1):length()
  end

  vec3.__index.length = function(v)
    --local ori = math.sqrt(v.x * v.x + v.y * v.y + v.z * v.z)
    --local sh4 = math.sqrt(sh4_sum_sq(v.x, v.y, v.z))
    --print("LEN: " .. sh4 .. " " .. ori)
    return math.sqrt(sh4_sum_sq(v.x, v.y, v.z))
  end

  vec3.__index.normalize = function(v, out)
    out = out or v
    local len = v:length()
    return len == 0 and v or v:scale(sh4_fsrra(sh4_sum_sq(v.x, v.y, v.z)), out)
  end
 
  vec3.__index.normalize = function(v, out)
    local out = out or v
    out.x, out.y, out.z = sh4_vecNormalize(v.x, v.y, v.z)
    return out
  end

  vec3.__index.lerp = function(v, u, t, out)
    out = out or v
